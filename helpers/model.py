    ...
    def predict_masked_token(self, text, return_label_distractor_prob=True, return_most_prob=True,):
        with torch.no_grad():
            ...
            if return_label_distractor_prob:
                returns.append(predictions_softmax[0].item() + predictions_softmax[1].item())
                returns.append(predictions_softmax[2].item() + predictions_softmax[3].item())
                returns.append(returns[-2]-returns[-1])
